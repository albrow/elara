...
impl BigEvilRoverActor {
    pub fn new(index: usize, bounds: Bounds) -> BigEvilRoverActor {
        BigEvilRoverActor { index, bounds }
    }
    ...
    fn bump_or_turn(
        ...
        if curr_orientation == desired_direction {
            return BigEvilRoverAction::Bump(self.forward_pos(state));
        }
        if desired_direction == self.left_direction(curr_orientation) {
            return BigEvilRoverAction::Turn(TurnDirection::Left);
        }
...
impl Actor for BigEvilRoverActor {
...
mod test {
    ...
    fn get_next_action() {
        let mut base_state = StateMaker::new()
            ...
            .build();
        let actor = BigEvilRoverActor::new(
            0,
            Bounds {
                min_x: 0,
                max_x: WIDTH as i32,
