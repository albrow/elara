...
<RunnableExample
  code={`// The add function outputs the sum of two numbers, which we
// then pass as the input to the say function.
say(add(2, 3));
say(add(5, 7));
`}
/>

Let's take a look at a different function called `get_orientation`. It outputs a string
