...
number between 1 and 100. We can use that output as the _input_ to
the `say` function:
...
As another example, we can also assign the output of the `random`
function to a variable called `number`. Then we can use the `number`
variable as the input to the `say` function:
